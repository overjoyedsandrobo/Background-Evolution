...
from prototypes_v4_fitted import PROTOTYPE_LIBRARY, PROTO_NAMES, PROTO_VECS, PROTO_GENS, ALL_TRAITS
...
_BASE_TRAIT_DEFS = {
    "air": _ft(temperature=0.40, pressure=0.00, gravity=0.00, solidity=0.00,
               humidity=0.00, toxicity=0.00, windspeed=1.00, visibility=1.00,
               luminosity=1.00, radiation=0.05, magnetism=0.00, volatility=1.00,
               flora=0.00, fauna=0.00, decay=0.00, age=0.00),
    "earth": _ft(temperature=0.60, pressure=1.00, gravity=1.00, solidity=1.00,
                 humidity=0.05, toxicity=0.35, windspeed=0.00, visibility=0.00,
                 luminosity=0.00, radiation=0.40, magnetism=0.60, volatility=0.00,
                 flora=0.00, fauna=0.00, decay=0.00, age=1.00),
    "water": _ft(temperature=0.15, pressure=0.70, gravity=0.60, solidity=0.02,
                 humidity=1.00, toxicity=0.20, windspeed=0.05, visibility=0.10,
                 luminosity=0.05, radiation=0.00, magnetism=0.05, volatility=0.10,
                 flora=1.00, fauna=1.00, decay=0.80, age=0.75),
    "fire": _ft(temperature=1.00, pressure=0.30, gravity=0.10, solidity=0.00,
                humidity=0.00, toxicity=0.70, windspeed=0.60, visibility=0.20,
                luminosity=0.95, radiation=0.85, magnetism=0.05, volatility=1.00,
                flora=0.00, fauna=0.00, decay=0.00, age=0.00),
}
...
def _blend_traits(parent_traits, ratios, child_gen):
    means = {t: sum(pt.get(t,0)*r for pt,r in zip(parent_traits, ratios))
             for t in ALL_TRAITS}
    ...
    variances = {t: sum(r*(pt.get(t,0)-means[t])**2 
...
if __name__ == "__main__":
    ...
    for parents, ratios, label in tests:
        ...
        clean = [n.rsplit('_',1)[0] if n.rsplit('_',1)[-1].isdigit() else n 
                 for n in results]
